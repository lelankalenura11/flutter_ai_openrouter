...
- **File picking**: `file_picker` package, using `FileType.custom` with `allowedExtensions: ['pdf']` for PDFs (and `FileType.image`/`FileType.video`/`FileType.audio` for other input types).
  - Setup: add to `pubspec.yaml`, check [pub.dev/packages/file_picker](https://pub.dev/packages/file_picker) for the current version before adding.
  - Android: add a `<queries>` block to `AndroidManifest.xml` for intent visibility on Android 11+ (required or picking can fail silently) — check the package's current setup docs, as manifest requirements shift between major versions.
  - No runtime storage permission (`READ_EXTERNAL_STORAGE`) is needed on modern Android (10+) or iOS — the package uses the OS's native document picker (Android SAF `ACTION_OPEN_DOCUMENT`, iOS `UIDocumentPickerViewController`/`PHPickerViewController`), so the user's file selection itself is the permission grant, scoped to that file only.
  - `result.files.single.path` gives a usable file path on Android/iOS/Windows; on web it's null and `.bytes` must be used instead (not relevant for this app's target platforms, but worth knowing if web support is ever added).
